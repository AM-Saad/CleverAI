...
AI-powered flashcards, quizzes, summaries, handwriting recognition, transcription, translation, and language stories use one OpenRouter adapter.
...
| `POST /api/ai/math-recognize`       | Handwriting to LaTeX   |
| `POST /api/ai/transcribe`           | Audio transcription    |
| `POST /api/language/translate`      | Lexical translation    |
...
`LlmUsage` stores OpenRouter's requested and actual model, native tokens/cost, latency, cache status, and errors. No model registry or price table exists. See [LLM_GENERATION_FLOW.md](./LLM_GENERATION_FLOW.md).
...
Inference runs server-side through OpenRouter. Browser speech synthesis remains a native, non-model capability.

### Composables

| Composable             | Purpose                                     |
| ---------------------- | ------------------------------------------- |
| `useMathRecognition`   | Canvas image to OpenRouter math recognition |
| `useSpeechToText`      | Recorded audio to OpenRouter transcription  |
| `useTextSummarization` | Text to OpenRouter summary                  |
| `useTextToSpeech`      | Native browser speech synthesis             |
