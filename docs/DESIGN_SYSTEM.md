...
| Focus | Use one visible, inset indicator per control. Fields use `focus:ring-2 focus:ring-inset focus:ring-[var(--ds-focus-outline-color)]`; buttons and button-like controls use a 2px tokenized inset outline. The native-control fallback is inset too, so focus remains visible inside scrolling and clipped containers. |
