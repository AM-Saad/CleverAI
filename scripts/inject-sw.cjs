...
function refreshNitroPublicAssetMetadata(swDest) {
  const nitroManifest = path.resolve(
    outputRoot,
  ...
  for (
    ...
    index < Math.min(entryStart + 10, lines.length);
    index += 1
  ) {
  ...
  if (!updatedEtag || !updatedMtime || !updatedSize) {
    throw new Error(
      "Could not refresh all /sw.js metadata in Nitro asset manifest",
    );
  }
...
async function run() {
  ...
  const swDest = path.resolve(outputRoot, "public/sw.js");
  const globDirectory = path.resolve(outputRoot, "public");
