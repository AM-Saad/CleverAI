...
export function useNotes(folderId: string) {
  ...
  const removeOperation = useOperation<unknown>();

  // Centralized createNote that lets FetchFactory handle all error construction
  const createNote = async (noteData: typeof CreateNoteDTO) => {
  ...
  const updateNote = async (id: string, noteData: typeof UpdateNoteDTO) => {
