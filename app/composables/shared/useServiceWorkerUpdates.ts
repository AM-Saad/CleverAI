...
export function useServiceWorkerUpdates() {
  ...
  const forceServiceWorkerUpdate = async () => {
    if (!isDev) {
      console.warn('forceServiceWorkerUpdate is only available in development mode')
      return
    }
    ...
    try {
      ...
      console.log('🔄 [DEV] Forcing page reload to re-register service worker...')
      setTimeout(() => {
  ...
  const forceServiceWorkerControl = async () => {
    if (!isDev) {
      console.warn('forceServiceWorkerControl is only available in development mode')
      return
    }
      ...
      if (registration.active && !navigator.serviceWorker.controller) {
        ...
        setTimeout(() => {
          if (navigator.serviceWorker.controller) {
            ...
            console.log('⚠️ [DEV] Service worker still not controlling - try refreshing manually')
          }
        }, 1000)
  ...
  const debugServiceWorker = async () => {
    ...
    try {
      ...
      if (navigator.serviceWorker.controller) {
        console.log('🔍 [DEV] Controller script URL:', navigator.serviceWorker.controller.scriptURL)
        console.log('🔍 [DEV] Controller state:', navigator.serviceWorker.controller.state)
      }
    } catch (error) {
  ...
  const testServiceWorkerMessage = async () => {
    if (!isDev) {
      console.warn('testServiceWorkerMessage is only available in development mode')
      return
    }
  ...
  const simulateUpdateAvailable = () => {
    if (!isDev) {
      console.warn('simulateUpdateAvailable is only available in development mode')
      return
    }
  ...
  const checkForUpdates = async () => {
    ...
    try {
      ...
      updateError.value = error instanceof Error ? error.message : 'Update check failed'
    }
  }
