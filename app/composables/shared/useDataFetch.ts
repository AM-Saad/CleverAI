 ...
 */
export function useDataFetch<T>(key: string, fetcher: () => Promise<Result<T>>) {
  const { data: result, pending, refresh, execute } = useAsyncData<Result<T>>(key, fetcher)
  ...
  const error = computed(() => result.value?.success === false ? result.value.error : null)
