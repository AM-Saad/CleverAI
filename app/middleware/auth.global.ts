...
export default defineNuxtRouteMiddleware((to) => {
  ...
  console.log('🔐 [AUTH MIDDLEWARE] TEMPORARILY DISABLED - allowing all routes');
  ...
  if (to.meta.auth === false) {
    console.log('🔐 [AUTH MIDDLEWARE] Route has auth: false, skipping auth check');
    return
  }

  // If authentication is still loading, allow page to render but don't redirect
  if (status.value === 'loading') {
    console.log('🔐 [AUTH MIDDLEWARE] Auth status is loading, allowing page to render anyway');
    // Don't block the render, just log and continue
    // Add a timeout to prevent infinite loading
    setTimeout(() => {
      if (status.value === 'loading') {
        console.warn('🔐 [AUTH MIDDLEWARE] Authentication took too long, redirecting to sign in')
  ...
  if (status.value === 'unauthenticated') {
    console.log('🔐 [AUTH MIDDLEWARE] User not authenticated, redirecting to sign in')
  ...
  if (requiredRole && user && (!user.role || user.role !== requiredRole)) {
    console.warn(`🔐 [AUTH MIDDLEWARE] User role ${user.role} does not match required role ${requiredRole}`)
    return navigateTo('/')
  }
