        ...
        <div class="max-w-7xl mx-auto px-4 py-8">
            ...
            <div class="mb-8">
                ...
                <p class="text-gray-600 dark:text-gray-400">
                    Comprehensive debugging tools for notifications, service workers, and system testing
                </p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <UCard>
                    ...
                    <div class="space-y-2">
                        <div class="flex justify-between">
                            ...
                            <UBadge :color="notificationStatus.permission === 'granted' ? 'success' : 'error'"
                                variant="subtle">
                ...
                <UCard>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex justify-between">
                            <span>Last Run:</span>
                            <span class="text-sm text-gray-600">{{ cronStatus.lastRun || 'Never' }}</span>
                        </div>
                    </div>
                </UCard>
            </div>
            ...
            <UTabs v-model="selectedTab" :items="tabs" class="w-full">
                ...
                <template #service-worker="{ item }">
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <UCard>
                            <template #header>
                                <h3 class="text-lg font-semibold">⚙️ Service Worker Management</h3>
                            </template>
                            <div class="space-y-4">
                        ...
                        <UCard>
                            ...
                            <div class="space-y-4">
                                ...
                                <div v-if="subscriptionInfo" class="mt-4 p-3 bg-gray-100 dark:bg-gray-800 rounded">
                                    <p class="text-sm font-mono break-all">{{ subscriptionInfo.endpoint }}</p>
                                </div>
                            </div>
                        </UCard>
                    </div>
                        ...
                        <UCard>
                            <template #header>
                                <div class="flex items-center justify-between">
                                    ...
                                    <UButton color="success" size="xs" :disabled="logMonitoring"
...
const tabs = [
    ...
    { key: 'service-worker', label: 'Service Worker', icon: 'i-heroicons-cog-6-tooth' },
...
const setResult = (operation: string, success: boolean, data?: any, message?: string) => {
    lastResult.value = {
        operation,
        success,
        data,
...
const testDirectNotification = async () => {
    ...
    try {
        ...
        setResult('Test Direct Notification', true, { title: testNotification.title })
    } catch (error: any) {
...
const checkServiceWorker = async () => {
    ...
    try {
        if ('serviceWorker' in navigator) {
            ...
            setResult('Check Service Worker', true, { registered: swStatus.registered, state: swStatus.state })
        } else {
...
const triggerCronCheck = async () => {
    loading.cron = true
    try {
        const response: any = await $fetch('/api/notifications/cron/check-due-cards', { method: 'GET' })
        addLog('success', `Cron triggered: ${response.data.results.notificationsSent} sent, ${response.data.results.skipped} skipped`)
    ...
    try {
        if ('serviceWorker' in navigator) {
            ...
            if (registration) {
                ...
                addLog('info', `Subscription status: ${subscription ? 'Active' : 'None'}`)
