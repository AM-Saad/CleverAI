...
const groupItemsByColumn = (boardItems: BoardItemState[]) => {
  const grouped = new Map<string | null, BoardItemState[]>();

  for (const item of boardItems) {
    const columnId = item.columnId ?? null;
  ...
  grouped.forEach((columnItems) => {
    columnItems.sort(sortItemsByOrder);
  });
...
const toggleCurrentItemFullscreen = () => {
  if (!currentItem.value) return;
  fullscreen.toggle(currentItem.value.id);
};
...
onMounted(async () => {
  try {
    ...
    const shouldSyncItems =
      !itemsStore.lastSync.value &&
      !(itemsStore.loadingStates.value.get("global") ?? false);
    const shouldSyncColumns =
      !columnsStore.lastSync.value &&
      !(columnsStore.loadingStates.value.get("global") ?? false);
...
const retryFailedBoardChanges = async () => {
  ...
  if (failedBoardColumnCount.value > 0) {
    await columnsStore.syncWithServer();
  }
};

const handleSyncStatusAction = async () => {
  if (boardFailedCount.value > 0) {
    await retryFailedBoardChanges();
    return;
  }
  ...
  <ui-card size="sm" variant="default" shadow="none"
    ...
    <template #default>
      ...
      <LocalSyncStatus
        ...
        class="mx-4 mt-3"
        feature-label="Board"
        :pending-count="boardPendingCount"
        :error-count="boardFailedCount"
        ...
        :last-sync="boardLastSync"
        :action-label="boardFailedCount > 0 ? 'Retry failed' : 'Sync now'"
