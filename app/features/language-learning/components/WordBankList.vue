    ...
    <div v-else class="space-y-3">
      <ui-card v-for="word in filteredWords" :key="word.id" variant="default">
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0 flex-1">
            ...
            <div class="mt-1 flex flex-wrap items-center gap-2">
              ...
              <span v-if="word.translation" class="text-sm font-medium text-primary">
                {{ word.translation }}
                {{
        ...
        <div v-if="word.meanings?.length" class="mt-3 space-y-1.5">
          <p v-for="(meaning, index) in word.meanings.slice(0, 2)" :key="`${word.id}-meaning-${index}`"
        ...
        <ui-card v-if="storyFor(word)" class="mt-3" variant="surface-strong" size="xs">
          <button type="button" class="flex w-full items-center justify-between gap-3 px-3 py-2 text-left"
            ...
            <Icon :name="expandedStoryIds.has(word.id)
              ? 'i-lucide-chevron-up'
