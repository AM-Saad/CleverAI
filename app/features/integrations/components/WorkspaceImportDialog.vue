...
const providerCards = computed(() => [
  {
    ...
    connected: accounts.value.filter((account) => account.provider === "jira")
      .length,
  },
  {
    ...
    connected: accounts.value.filter((account) => account.provider === "notion")
      .length,
  },
]);
...
const sourceCards = computed(() =>
  sources.value.map((source) => ({
    ...
    kindLabel:
      ...
      source.supportedKinds.includes("DOCUMENT")
        ? "Tasks + docs"
        : source.supportedKinds.includes("TASK")
          ? "Tasks"
          : "Docs",
...
const previewCountLabel = computed(() => {
  ...
  if (targetType.value === "NOTE")
    return `${docCount} document${docCount === 1 ? "" : "s"} ready`;
...
const importActionLabel = computed(() => {
  if (isImporting.value)
    return targetType.value === "NOTE" ? "Importing docs" : "Importing tasks";
  return targetType.value === "NOTE" ? "Import docs" : "Import tasks";
});
const canPreview = computed(
  () =>
    Boolean(selectedAccount.value && selectedSource.value) &&
    !isPreviewing.value,
);
...
const resultStats = computed(() => {
  ...
  return [
    ...
    {
      label: "Conflicts",
      value: result.value.conflicted,
      tone: result.value.conflicted
        ? ("warning" as const)
        : ("neutral" as const),
    },
  ];
...
const { onKeydown: onDialogKeydown } = useFocusTrap(isOpen, dialogPanelEl, {
...
async function loadAccounts() {
  ...
  try {
    ...
    if (!response.success) {
      error.value =
        response.error?.message || "Could not load connected accounts";
      return;
    }
...
async function loadMappings() {
  ...
  try {
    const response = await $api.workspaceIntegrations.getMappings(
      props.workspaceId,
    );
    if (response.success) {
...
async function loadSources(accountId: string) {
  ...
  try {
    ...
    if (!response.success) {
      error.value =
        response.error?.message || "Could not load integration sources";
      return;
    }
    ...
    error.value =
      err instanceof Error ? err.message : "Could not load integration sources";
  } finally {
...
async function importSelectedSource() {
  ...
  try {
    const response = await $api.workspaceIntegrations.runImport({
      ...
      contentKinds: [...selectedContentKinds.value],
      noteGroupTitle:
        noteGroupTitle.value ||
        `Imported from ${providerName(selectedSource.value.provider)}`,
      limit: limit.value,
      fieldMapping: fieldMapping(),
      importOptions: {
    ...
    toast.add({
      title:
        targetType.value === "NOTE" ? "Documents imported" : "Tasks imported",
...
async function refreshMapping(mapping: WorkspaceImportMappingSummary) {
  ...
  error.value = null;
  try {
    const response = await $api.workspaceIntegrations.refreshImport({
...
watch(selectedSourceId, (sourceId) => {
  resetPreview();
  applySourceDefaults(
    sources.value.find((source) => source.id === sourceId) ?? null,
  );
});
    ...
    <Teleport to="body">
      ...
      >
        ...
        >
          ...
          >
            <header
              class="flex items-start justify-between gap-4 border-b border-secondary bg-surface px-5 py-4"
            >
              <div class="flex min-w-0 items-start gap-3">
                <span
                  class="mt-0.5 inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-[var(--radius-md)] bg-surface ring-1 ring-secondary"
                >
                  <IntegrationAppsIcon class="h-6 w-6" />
                </span>
                <div class="min-w-0">
                  <p
                    class="text-xs font-semibold uppercase tracking-widest text-content-secondary"
                  >
                    Apps
                  </p>
                  <ui-title
                    ...
                    class="truncate text-lg font-semibold text-content-on-surface"
                  >
            ...
            <div class="min-h-0 flex-1 space-y-5 overflow-y-auto px-5 py-5">
              <div class="grid gap-3 sm:grid-cols-2">
                ...
                >
                  <span class="flex min-w-0 items-center gap-3">
                    <span
                      class="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-[var(--radius-md)] bg-surface text-primary ring-1 ring-secondary transition group-hover:bg-primary/10 group-hover:ring-primary/30"
                    >
                    ...
                    <span class="min-w-0">
                      <span
                        class="block truncate text-sm font-semibold text-content-on-surface"
                        >{{ card.name }}</span
                      >
                      <span
                        class="mt-0.5 block truncate text-xs text-content-secondary"
                  ...
                  <Icon
                    ...
                    class="h-4 w-4 shrink-0 text-content-secondary transition group-hover:text-primary"
                  />
                </UiInteractiveCard>
              </div>
              ...
              <UiPanel variant="subtle" size="md" content-class="space-y-4">
                <div
                  class="flex items-center justify-between gap-3 border-b border-secondary pb-3"
                >
                  <div class="min-w-0">
                    <p
                      class="truncate text-sm font-semibold text-content-on-surface"
                    >
                ...
                >
                  ...
                  >
                    <span class="flex min-w-0 items-center gap-3">
                      <span
                        class="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-[var(--radius-md)] bg-surface-subtle ring-1 ring-secondary"
                      >
                        <IntegrationProviderLogo
                      ...
                      <span class="min-w-0">
                        ...
                        >
                        <span
                          class="mt-0.5 flex flex-wrap items-center gap-1.5 text-xs text-content-secondary"
                        >
                          <span v-if="source.key">{{ source.key }}</span>
                          <UiBadge tone="neutral" variant="soft" size="xs">{{
                            source.kindLabel
                          }}</UiBadge>
                        </span>
                      </span>
                ...
                <UiPanel
                  v-else-if="selectedAccountId && !isLoadingSources"
                  variant="subtle"
                  class-name="p-3"
                >
                  <p class="text-sm font-medium text-content-on-surface">
                    No sources found
                  </p>
                  <p class="mt-1 text-xs text-content-secondary">
                    Reconnect with workspace/page access, then refresh accounts.
                  </p>
                </UiPanel>
                ...
                <div class="grid gap-3 md:grid-cols-2">
                  ...
                  >
                    ...
                    <span class="min-w-0">
                      <span
                        class="block text-sm font-semibold text-content-on-surface"
                        >Board tasks</span
                      >
                      <span class="block text-xs text-content-secondary"
                        >Jira issues or task-like Notion rows</span
                      >
                    </span>
                  </UiInteractiveCard>
                  ...
                  >
                    ...
                    <span class="min-w-0">
                      ...
                      >
                      <span class="block text-xs text-content-secondary"
                        >Notion pages into a note group</span
                      >
                    </span>
                  </UiInteractiveCard>
                </div>
                ...
                <div v-if="showAdvanced" class="grid gap-3 md:grid-cols-2">
                  ...
                  <label
                    v-if="selectedAccount?.provider === 'jira'"
                ...
                <UiPanel
                  ...
                  content-class="flex items-start gap-2 text-error-text"
                >
                  <Icon
              ...
              <UiPanel variant="surface" size="md">
                ...
                <div v-if="mappings.length" class="mt-4 space-y-2">
                  ...
                  >
                    <div class="flex min-w-0 items-start gap-3">
                      <span
                        class="mt-0.5 inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-[var(--radius-md)] bg-surface ring-1 ring-secondary"
                      >
                        <IntegrationProviderLogo
                      ...
                      <div class="min-w-0">
                        <p
                          class="truncate text-sm font-medium text-content-on-surface"
                        >
                          {{ mapping.name }}
                        </p>
                        <p class="mt-0.5 text-xs text-content-secondary">
                          ...
                          {{
                            mapping.targetType === "NOTE" ? "Notes" : "Board"
                          }}
                          · {{ mapping.refCounts.total }} imported
                        </p>
                        <div class="mt-2 flex flex-wrap gap-1.5">
                          <UiBadge
                            v-if="mapping.refCounts.conflicted"
                            tone="warning"
                            size="xs"
                          >
                          ...
                          <UiBadge
                            v-if="mapping.refCounts.localChanged"
                            tone="neutral"
                            size="xs"
                          >
                          ...
                          <UiBadge
                            v-if="
                              !mapping.refCounts.conflicted &&
                              !mapping.refCounts.localChanged &&
                              !mapping.refCounts.error
                            "
                            tone="success"
                            size="xs"
                          >
              ...
              <UiPanel variant="surface" size="md">
                ...
                <div v-if="isPreviewing" class="mt-4 grid gap-2">
                  <UiSkeleton
                    v-for="index in 3"
                    :key="index"
                    class="h-14 rounded-[var(--radius-md)]"
                  />
                </div>
                ...
                <div v-else-if="preview" class="mt-4 space-y-3">
                  ...
                  >
                    ...
                    <p
                      v-if="'excerpt' in item && item.excerpt"
                      class="mt-1 line-clamp-2 text-xs text-content-secondary"
                    >
                      {{ item.excerpt }}
                    </p>
                  </UiPanel>
                  <UiPanel
                    ...
                    content-class="text-xs text-warning-text"
                  >
                ...
                <UiPanel v-else variant="subtle" class-name="mt-4 p-3">
                  ...
                  <p class="mt-1 text-xs text-content-secondary">
                    Check sample rows, permissions, and warnings before writing
                    to Board or Notes.
                  </p>
                </UiPanel>
              </UiPanel>
              ...
              <UiPanel
                ...
                variant="subtle"
                size="md"
                class-name="border-success/20 bg-success/10"
              >
                ...
                <div class="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
                  <UiPanel
                    ...
                    size="xs"
                  >
                    <p class="text-lg font-semibold text-content-on-surface">
                      {{ stat.value }}
                    </p>
                    <UiBadge :tone="stat.tone" size="xs" variant="soft">{{
                      stat.label
                    }}</UiBadge>
                  </UiPanel>
                </div>
            ...
            <footer
              class="flex flex-col gap-3 border-t border-secondary bg-surface px-5 py-4 sm:flex-row sm:items-center sm:justify-between"
            >
