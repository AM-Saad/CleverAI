...
export default defineNuxtPlugin((nuxtApp) => {
  ...
  nuxtApp.vueApp.config.errorHandler = (error: unknown, instance: ComponentPublicInstance | null, info: string) => {
    ...
    if (instance) {
      const componentName = instance.$.type?.name || instance.$.type?.__name || 'Unknown';
      console.error("Component:", componentName);
      console.error("Component Props:", instance.$props);
    }

    // Handle specific skeleton/slot rendering errors
    if (error instanceof Error && error.message?.includes("Cannot read properties of null") && info === 'render function') {
      console.warn("💀 Skeleton/Slot rendering error detected - this might be a timing issue with component initialization");
      console.warn("💡 Suggestion: Check if slots are properly guarded with v-if or provide default content");
    }
  ...
  nuxtApp.hook('vue:error', (error: unknown, instance: ComponentPublicInstance | null, info: string) => {
    ...
    if (error instanceof Error && error.message?.includes('renderSlot')) {
      console.warn("⚠️ Slot rendering issue - check for proper slot usage and v-if conditions");
      console.warn("🔍 Common fixes:");
      console.warn("  - Ensure slots have fallback content: <slot>Default content</slot>");
      console.warn("  - Guard conditional slots: <template v-if='condition'><slot /></template>");
