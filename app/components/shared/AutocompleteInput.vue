  ...
  <div ref="rootRef" class="relative w-full">
    ...
    <div class="relative">
      ...
      <UiInput
        ...
        v-bind="$attrs"
        :class="['w-full', ghostSuffix ? 'caret-primary' : '']"
        autocomplete="off"
        autocorrect="off"
        spellcheck="false"
...
const ghostSuffix = computed(() => {
  ...
  if (
    !currentWord ||
    !first.toLowerCase().startsWith(currentWord.toLowerCase())
  )
    return "";
...
function handleInput(val: string | number | null | undefined) {
  const nextValue = val == null ? "" : String(val);
  activeIndex.value = 0;
  emit("update:modelValue", nextValue);
  emit("query", nextValue);
}
