<template>
  <Teleport to="body">
    <div
      v-if="show"
      ref="dialogEl"
      class="fixed inset-0 z-[var(--z-modal)] flex items-center p-4 justify-center overflow-hidden"
      role="dialog"
      aria-modal="true"
      aria-labelledby="learning-hub-modal-title"
      tabindex="-1"
      @keydown="onKeydown"
    >
      ...
      >
        <div
          v-if="show"
          class="absolute inset-0 bg-[var(--ds-backdrop-dim)] backdrop-blur-md"
          @click="handleClose"
        ></div>
      </Transition>
        ...
        >
          ...
          <div class="flex-1 overflow-hidden h-full flex flex-col">
            <LearningHubContent
              ...
              :is-enrolling-loading="enrollmentLoading"
              :enrolled-flashcard-ids="enrolledFlashcardIds"
              :enrolled-question-ids="enrolledQuestionIds"
              :updating="updating"
