...
import { useDrawerMotion } from '~/composables/ui/useDrawerMotion'
...
const isMobile = computed(() => props.mobile === 'auto' ? mq.value : !!props.mobile)
...
const mode = computed(() => {
    ...
    const constraints = props.side === 'right'
...
function isInteractive(el: EventTarget | null): boolean {
    ...
    if (role && /^(textbox|combobox|button|link|switch|slider)$/.test(role)) return true
    // If inside an interactive ancestor
    return !!node.closest('input, textarea, select, [contenteditable="true"], button, a, [role="textbox"], [role="combobox"], [role="button"], [role="link"], [role="switch"], [role="slider"]')
}

function onPointerDownCapture(e: PointerEvent) {
    suspendDrag.value = isInteractive(e.target)
}
function onPointerUpCapture() { suspendDrag.value = false }
function onPointerCancelCapture() { suspendDrag.value = false }
...
function snapTo(pos: number) { requestAnimationFrame(() => { if (alive) targetPos.value = pos }) }

onMounted(() => {
    requestAnimationFrame(() => {
        drawer.recompute()
        sheet.recompute()
        requestAnimationFrame(() => { snapTo(props.show ? mode.value.open : mode.value.closed) })
    })
})

watch(() => props.show, (v) => { snapTo(v ? mode.value.open : mode.value.closed) })

watch(() => props.show, (v) => {
    if (!props.lockScroll) return
    const root = document.documentElement
    const body = document.body
    if (v) { root.style.overflow = 'hidden'; body.style.overflow = 'hidden' }
...
const { onKeydown } = useFocusTrap(
    computed(() => props.show),
    panelEl,
    { onEscape: () => { snapTo(mode.value.closed); emit('closed') } }
)

const transitionProps = { type: 'spring' as const, stiffness: 600, damping: 70 }

interface DragInfo { offset: { x: number; y: number }; velocity: { x: number; y: number } }
function handleDragStart() { startPos.value = targetPos.value }
function handleDragEnd(_: Event, info: DragInfo) {
    ...
    else { snapTo(mode.value.closed); emit('closed') }
}
</script>
