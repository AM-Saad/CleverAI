<template>
    <div :class="cn('h-72 w-64 select-none [perspective:1000px]', props.class)">
        <div :class="cn(
            'relative h-full rounded-2xl transition-transform duration-500 will-change-transform cursor-pointer text-slate-200 [transform-style:preserve-3d]'
        )" :style="{ transform: wrapperTransform }" @click="flipped = !flipped">
            <!-- Front -->
            <div
                class="absolute size-full overflow-hidden rounded-2xl border [backface-visibility:hidden] bg-foreground p-4">
                <slot name="front" />
            </div>
