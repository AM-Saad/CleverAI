<template>
    <header :class="['border-b py-4 mb-4', borderClass]">
        <nav class="flex gap-x-5">
            <button v-for="(tab, index) in items" :key="index" type="button"
                :class="[buttonBaseClass, activeIndexLocal === index ? activeClass : inactiveClass]"
...
const props = defineProps({
    ...
    inactiveClass: {
        type: String,
        default: 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-300',
    },
    buttonBaseClass: {
        type: String,
