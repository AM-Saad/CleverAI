...
export const NoteLayoutChangeSchema = z.object({
...
export const NotesSyncRequestSchema = z.object({
  ...
  {
    message: "Sync request must include note changes, group changes, or a layout change",
  },
);
...
export const NotesSyncResponseSchema = z.object({
  ...
  errors: z.array(z.object({ scope: z.string(), id: z.string().optional(), message: z.string() })).default([]),
  layoutApplied: z.boolean().default(false),
  layoutConflict: z.boolean().default(false),
}).transform((response) => ({
  ...response,
  noteIdMap: Object.keys(response.noteIdMap).length ? response.noteIdMap : response.idMap,
  idMap: Object.keys(response.idMap).length ? response.idMap : response.noteIdMap,
}));
