...
export default defineEventHandler(async (event) => {
  ...
  if (!attemptId || !questionId) {
    ...
    return { error: 'attemptId and questionId are required.' }
  }
  ...
  if (currentIndex !== nextUnansweredIndex) {
    setResponseStatus(event, 409)
    return { error: 'You must answer questions in order.' }
  }
  answers[questionId] = answer
  await prisma.examAttempt.update({ where: { id: attemptId }, data: { answers } })
  if (currentIndex + 1 < questions.length) {
    return { nextQuestion: questions[currentIndex + 1], questionIndex: currentIndex + 1, totalQuestions: questions.length }
  } else {
    return { message: 'All questions answered. Please submit the exam.' }
  }
})
