  ...
  for (const change of parsed.changes) {
    try {
      if (change.operation === 'delete') {
        ...
        const note = await prisma.note.findFirst({ where: { id: change.id, folder: { userId: user.id } } })
        if (!note) {
      ...
      // upsert
      const folder = change.folderId ? await prisma.folder.findFirst({ where: { id: change.folderId, userId: user.id } }) : null
      ...
      const existing = await prisma.note.findFirst({ where: { id: change.id, folder: { userId: user.id } } })

      if (!existing) {
        // Create new note with provided content/folder
        if (!change.folderId) { conflicts.push({ id: change.id }); continue }
        await prisma.note.create({ data: { id: change.id, folderId: change.folderId, content: change.content || '' } })
        applied.push(change.id)
      } else {
        // Conflict if server newer than client
        if (existing.updatedAt && existing.updatedAt.getTime() > change.updatedAt) {
          conflicts.push({ id: change.id })
          continue
        }
        await prisma.note.update({ where: { id: change.id }, data: { content: change.content ?? existing.content } })
        applied.push(change.id)
      }
    } catch (e) {
