...
async function handlePostRequest(event: any, path: string) {
  ...
  switch (path) {
    ...
    case "alerts/init": {
      ...
      return success({ alertIds }, { message: "Default alerts initialized" });
    }

    case "alerts/common": {
      const alertType = body.type as keyof typeof CommonAlerts;
      if (!CommonAlerts[alertType])
        throw Errors.badRequest("Invalid alert type");
...
function getHealthRecommendations(
  ...
  if (errorRate.errorRate > 5) {
    recommendations.push(
      "High error rate detected - investigate recent deployments"
    );
  }
  ...
  if (highImpactOps.length > 0) {
    recommendations.push(
      `High impact operations detected: ${highImpactOps.map((op) => op.operation).join(", ")}`
    );
  }
