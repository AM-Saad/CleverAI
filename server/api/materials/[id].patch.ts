...
export default defineEventHandler(async (event) => {
  ...
  if (typeof data.llmPrompt === 'string' || data.llmPrompt === null) updateData.llmPrompt = data.llmPrompt
  if (data.metadata !== undefined) updateData.metadata = data.metadata ? JSON.parse(JSON.stringify(data.metadata)) : null
