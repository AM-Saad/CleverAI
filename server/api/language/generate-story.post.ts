...
export default defineEventHandler(async (event) => {
  ...
  const relatedWords =
    data.relatedWords.length > 0
      ? [...data.relatedWords]
      : relatedCandidates.map((word: RelatedWordCandidate) => word.word);
  ...
  try {
    ...
    // Parse the story JSON before we finalize quota/logging.
    let parsed: {
      title?: string;
      storyText: string;
    ...
    // Log cost + latency and increment quota only after the payload is valid.
    ...
    await prisma.languageWord
      .update({
        where: { id: languageWord.id },
        data: { status: languageWord.status },
      })
      .catch(() => {});
