...
const baseStyles = {
  ...
  button: 'display: inline-block; color: white; text-decoration: none; padding: 12px 24px; border-radius: 6px; font-weight: bold;',
...
const brandInfo = {
  ...
  websiteUrl: process.env.APP_BASE_URL || 'http://localhost:3000'
}
 ...
 */
export const generateButton = (text: string, url: string, backgroundColor = '#2563eb'): string => {
  return `
 ...
 */
export const replaceVariables = (content: string, data: EmailTemplateData): string => {
  let result = content
