...
export default defineEventHandler(async (event) => {
  ...
  const existing = await prisma.folder.findFirst({ where: { id: id, userId: user.id } })
  if (!existing) {
  ...
  if (body.materialContent) {
    ...
    const existingMaterial = await prisma.material.findFirst({ where: { folderId: id, title: materialData.title } })
    if (existingMaterial) {
      await prisma.material.update({
        where: { id: existingMaterial.id },
        data: { content: materialData.content, type: materialData.type, llmModel: materialData.llmModel }
      })
    } else {
