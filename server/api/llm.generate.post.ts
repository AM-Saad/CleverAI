...
export default defineEventHandler(async (event) => {
  ...
  try {
    if (task === "flashcards") {
      ...
      event.node.res.setHeader(
        "x-llm-generated-count",
        String(flashcards.length)
      );
    ...
    const message =
      err instanceof Error && /quota/i.test(err.message)
        ? "Quota exceeded. Please check your OpenAI plan/billing or try again later."
        : "Generation failed. Please try again or switch model.";
