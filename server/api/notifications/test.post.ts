...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    try {
      validatedNotification = TestNotificationDTO.parse(body);
    } catch (e) {
    ...
    const results = await Promise.allSettled(
      subscriptions.map(async (sub: NotificationSubscription) => {
        try {
          ...
          console.error(
            `Failed to send test notification to subscription ${sub.id}:`,
            error
          );
    ...
    const sent = results.filter(
      (r: PromiseSettledResult<TestResult>) =>
    ...
    return success({
      ...
      results: results.map((r: PromiseSettledResult<TestResult>) =>
        r.status === "fulfilled"
          ? r.value
