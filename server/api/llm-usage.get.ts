...
export default defineEventHandler(async (event) => {
  ...
  const byMonth: Record<string, { usdMicros: bigint; calls: number; tokens: number }> = {}
  const byFeature: Record<string, { usdMicros: bigint; calls: number; tokens: number }> = {}
  const byUser: Record<string, { usdMicros: bigint; calls: number; tokens: number }> = {}
  ...
  const toArray = <T extends { usdMicros: bigint; calls: number; tokens: number }>(obj: Record<string, T>) =>
    Object.entries(obj)
      .map(([key, v]) => ({ key, usd: usdFromMicros(v.usdMicros), calls: v.calls, tokens: v.tokens }))
  ...
  const tokensPer1kChars = totalChars > 0 ? totalTokens / (totalChars / 1000) : null
