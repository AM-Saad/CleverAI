...
/**
 * Estimate tokens from text using improved heuristic
 * ~3.5 chars per token for English text
 * Add 10% overhead for markdown/code formatting
 */
export function estimateTokensFromText(text: string): number {
  const baseEstimate = Math.ceil(text.length / 3.5)
  const overhead = Math.ceil(baseEstimate * 0.1)
  return Math.max(10, baseEstimate + overhead)
}
