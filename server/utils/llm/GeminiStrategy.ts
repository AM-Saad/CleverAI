...
import type { LLMStrategy, FlashcardDTO, QuizQuestionDTO } from './LLMStrategy'
import { flashcardPrompt, quizPrompt } from '~~/server/utils/llm/prompts'
...
function stripCodeFences(s: string): string {
  ...
  return s.replace(/```[a-zA-Z]*\n([\s\S]*?)```/g, (_m, p1) => String(p1).trim())
}

function findBalancedSlice(text: string, open: string, close: string): string | null {
...
export class GeminiStrategy implements LLMStrategy {
  ...
  private async chatOnce(prompt: string): Promise<string> {
    ...
    if (process.env.GEMINI_MOCK === '1') {
      ...
      if (p.includes('question') || p.includes('choices') || p.includes('"answerindex"')) {
        // Quiz sample
        return JSON.stringify([
          { "question": "What is gravity?", "choices": ["A force", "A color"], "answerIndex": 0 },
          { "question": "Who formulated gravity laws?", "choices": ["Einstein", "Newton"], "answerIndex": 1 }
        ])
      } else {
        // Flashcards sample
        return JSON.stringify([
          { "front": "What is gravity?", "back": "A force that pulls objects together." },
    ...
    try {
      ...
      const model = this.client.getGenerativeModel({ model: GeminiStrategy.MODEL })
      ...
      try {
        const postCount = await (model as any).countTokens({
          contents: [{ role: 'model', parts: [{ text: typeof txt === 'string' ? txt : '' }] }],
        })
      ...
      this.onMeasure?.({
        ...
        requestId: undefined,
        rawUsage: usage,
        meta: { inputChars, outputChars, inputTokensEstimate, outputTokensEstimate },
      })
  ...
  async generateFlashcards(input: string): Promise<FlashcardDTO[]> {
    ...
    if (process.env.NODE_ENV === 'development' && (!raw || raw.length === 0)) {
      ...
      console.info('[Gemini debug] candidate(200)=', String(candidate).slice(0, 200))
    }
  ...
  async generateQuiz(input: string): Promise<QuizQuestionDTO[]> {
    ...
    for (const item of raw) {
      ...
      const choices = Array.isArray(item?.choices) ? item.choices.filter((c: any) => typeof c === 'string') : []
