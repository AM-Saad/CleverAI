...
export async function checkRedisLimit(key: string, max: number): Promise<{ used: boolean; remaining?: number }> {
  try {
    ...
    if (count > max) throw createError({ statusCode: 429, statusMessage: 'Rate limit exceeded. Please wait before trying again.' })
    return { used: true, remaining: Math.max(0, max - count) }
  } catch (e) {
    // Fall back to in-memory limiter on any Redis error
    console.warn('[rateLimit] Redis unavailable, falling back to memory:', (e as any)?.message || e)
    return { used: false }
  }
}
...
export function getClientIp(event: any): string {
  ...
  const socketIp = (event.node.req.socket && event.node.req.socket.remoteAddress) || (event.node.req.connection as any)?.remoteAddress || ''
  const rawIp = forwardedIp || socketIp || 'unknown'
  return rawIp.startsWith('::ffff:') ? rawIp.slice(7) : rawIp
}

export async function applyLimit(key: string, max: number, mem: MemCounter, now: number, windowMs: number): Promise<number> {
  ...
  if (rl.count >= max) throw createError({ statusCode: 429, statusMessage: 'Rate limit exceeded. Please wait before trying again.' })
  rl.count++
  return Math.max(0, max - rl.count)
}

export function setRateLimitHeaders(event: any, overallRemaining: number, userRemaining: number, ipRemaining: number, now: number) {
  ...
  if (overallRemaining === 0) event.node.res.setHeader('Retry-After', String(resetSeconds))
}
